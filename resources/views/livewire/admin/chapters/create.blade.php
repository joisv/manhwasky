            ...
            <div class="space-y-1">
                    ...
                    <div class="flex space-x-1 items-center w-full">
                        <div class="gap-1 flex flex-1 items-center ">
                            @foreach ($selectedSeries as $selected)
                                <button type="button" wire:click="restoreSeries({{ $selected['id'] }})"
                        ...
                        <div class="flex space-x-1 items-center w-full">
                            <input type="text" placeholder="Cari berdasarkan nama"
                                ...
                                wire:model.live.debounce.200ms="searchSeries" x-on:focus="selectShow = true">
                            <button type="button" @click="selectShow = ! selectShow">
                                <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none"
                                    class="ease-in duration-200" :class="selectShow ? 'rotate-180' : ''"
                        ...
                        <div class="w-full h-32 overflow-y-auto">
                            @forelse ($series as $index => $series)
                                <div id="series-{{ $index }}"
                                    ...
                                    <button type="button"
                                        wire:click="setSelectedSeries({{ $series->id }}, '{{ $series->title }}')"
                                        class="w-full text-start p-1 hover:bg-gray-200 ease-in duration-150">{{ $series->title }}</button>
                                </div>
                            @empty
                                <div class="text-base font-medium w-full text-center">Series tidak
                                    ada/ditemukan</div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="space-y-2 pb-2" :class="permadate ? 'border-b border-gray-400' : ''" wire:ignore>
                    ...
                    <div x-show="permadate" x-collapse class="space-y-2">
                        ...
                        {{-- tanggal --}}
                        <input id="flatpickr_chapter" wire:model="created" type="text"
