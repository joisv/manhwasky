<div x-data="{
    ...
    <x-tables.table name="Users" count="{{ $users->count() }} User">
        ...
        <x-slot name="tbody">
            @foreach ($users as $index => $user)
                <tr>
                    ...
                    <x-tables.td>{{ $user->updated_at->format('F j, Y') }}</x-tables.td>
                    {{-- <x-tables.td>{{ $user->category->name }}</x-tables.td> --}}
                    <x-tables.td>
                        {{-- <x-primary-button type="button" @click="editUser({{ $user->id }})">edit</x-primary-button> --}}
