<div x-data="{
    createCategory(){
        ...
        $dispatch('open-modal', 'category-create')
    },
    editCategory(id){

        $dispatch('open-modal', 'category-edit');
        $dispatch('edit', { value: id });

    }
}">
    <x-tables.table name="Categories" count="{{ $categories->count() }} Categories">
        ...
        <x-slot name="addBtn">
            <x-tables.addbtn type="button" x-data="" @click="createCategory">
                Add Categories
            </x-tables.addbtn>
        </x-slot>
        <x-slot name="sort">
        ...
        <x-slot name="tbody">
            @foreach ($categories as $index => $category)
                <tr>
                    ...
                    <x-tables.td>
                        <x-primary-button type="button" @click="editCategory({{ $category->id }})">edit</x-primary-button>
                        <x-danger-button type="button"
                            wire:click="destroyAlert({{ $category->id }}, 'delete')">delete</x-danger-button>
                    </x-tables.td>
                </tr>
            @endforeach
        </x-slot>
    ...
    <x-modal name="category-create" :show="$errors->isNotEmpty()" maxWidth="sm">
        <livewire:admin.categories.create />
    </x-modal>
    <x-modal name="category-edit" :show="$errors->isNotEmpty()" maxWidth="sm">
